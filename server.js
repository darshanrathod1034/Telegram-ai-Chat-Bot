...
const SERVER_URL = process.env.SERVER_URL || 'https://your-domain.com'; // Required for webhooks
...
async function sendDocument(chatId, buffer, filename = 'document.pdf') {
  try {
    ...
    // Fallback: send as photo if document fails
    try {
      ...
      console.error('Error sending photo fallback:', photoError.message);
    }
  }
}
...
// Option 2: Use Gemini 2.0 Flash API
...
// Main AI function - tries Gemini first, falls back to Ollama
async function askAI(prompt, preferProvider = 'gemini') {
  ...
  try {
    ...
    throw new Error(`Both AI providers failed. Gemini: ${GEMINI_API_KEY ? 'configured' : 'not configured'}. Ollama: ${ollamaError.message}`);
  }
}
  ...
  try {
    ...
    if (intent === 'resume') {
      ...
      try {
        ...
        await sendDocument(chatId, pdfBuffer, `${userName}_resume.pdf`);
      ...
      try {
        ...
        await sendDocument(chatId, pdfBuffer, `${userName}_Utara-request.pdf`);
      ...
      // General chat - use AI
      const systemPrompt = `You are a helpful AI assistant. Respond to the user's message in a friendly and concise manner. and u always need to greet the user for first time as "Jay Swaminirayan 🙏"`;
      ...
      try {
        ...
        await sendMessage(chatId, 
          ...
          `Make sure Gemini API is configured.\n\n` +
          `Error: ${error.message}`
        );
      }
    }
...
app.get('/', (req, res) => {
  res.json({
    ...
    providers: {
      ollama: OLLAMA_MODEL,
      gemini: GEMINI_API_KEY && GEMINI_API_KEY !== 'YOUR_GEMINI_API_KEY' ? 'configured' : 'not configured'
    }
  });
});
